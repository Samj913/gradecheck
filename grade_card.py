...
def launch_gui() -> None:
    ...
    def create_zoomable_preview(parent: ttk.LabelFrame) -> dict:
        ...
        state = {
            "canvas": canvas,
            "base_image": None,   # PIL.Image
            "tk_image": None,     # ImageTk.PhotoImage
            "canvas_item": None,  # item id
            "scale": 1.0,
        }
        ...
        def render() -> None:
            ...
            if state["canvas_item"] is None:
                state["canvas_item"] = canvas.create_image(0, 0, anchor="nw", image=tk_img)
            else:
                canvas.itemconfigure(state["canvas_item"], image=tk_img)
            canvas.configure(scrollregion=(0, 0, w, h))

        def set_image(pil_image: Optional[Image.Image]) -> None:
            state["base_image"] = pil_image
            state["scale"] = 1.0
        ...
        def zoom(event: tk.Event) -> None:
            ...
            state["scale"] = max(0.2, min(8.0, state["scale"] * factor))
            render()
    ...
    def set_preview(preview_state: dict, overlay_image: Optional[Image.Image]) -> None:
        preview_state["set_image"](overlay_image)
    ...
    def run_grading() -> None:
        ...
        try:
            ...
            set_preview(front_preview, front_overlay)

            if back_diag is not None:
                back_overlay = render_diagnostic_overlay(back_diag, "Back")
                set_preview(back_preview, back_overlay)
            else:
                set_preview(back_preview, None)
    ...
    front_preview = create_zoomable_preview(left_panel)
    back_preview = create_zoomable_preview(right_panel)
